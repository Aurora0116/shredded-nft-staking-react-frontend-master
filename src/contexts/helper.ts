...
import { DECIMALS, EPOCH, FACTOR, LEGENDARY_REWARD_AMOUNT, NORMAL_REWARD_AMOUNT } from '../config';
...
const PROGRAM_ID = "FtTLiU8ohFqghRNjKEtYfzs8PWtD1Tdk2W6eaA77c5Kb";
...
export const stakeNft = async (userAddress: PublicKey, mint: PublicKey, isLegendary: boolean, startLoading: Function, endLoading: Function, updatePageStates: Function) => {
    startLoading()
...
export const withdrawNft = async (userAddress: PublicKey, mint: PublicKey, startLoading: Function, endLoading: Function, updatePageStates: Function) => {
    let userTokenAccount = await getAssociatedTokenAccount(userAddress, mint);
    // console.log("Shred NFT = ", mint.toBase58(), userTokenAccount.toBase58());
    startLoading();
    try {
...
export const claimReward = async (userAddress: PublicKey, startLoading: Function, endLoading: Function) => {
    startLoading();
    try {
        let cloneWindow: any = window;
        let provider = new anchor.Provider(solConnection, cloneWindow['solana'], anchor.Provider.defaultOptions())
